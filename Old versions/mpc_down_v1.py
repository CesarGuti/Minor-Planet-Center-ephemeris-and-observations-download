...
===============================================================================
"""
# 2023-11-01 --> @Cesar Gutierrez de la Camara: RELEASE (python 3.9) (v1)
...
===============================================================================
"""
# Default values for form fields that if not sent give an error, irrelevant to us
...
def user_input():
    ...
    # Numer of days to try to output
    num_days = 0
    while True:
        num_days = input("Type the number of days to try to output: ")
        try:
            number = float(num_days)
            if number == 0:
                print("The number of days cannot be zero. Please try again.")
            elif number > 4001:
                print("The number of days cannot be greater than 4001")
            else:
                break  # Break the loop if the input is a valid number and not zero
        except ValueError:
            # Catch the ValueError if conversion to float fails, which means the input was not a number
            print("This is not a valid number. Please try again.")

    # Days interval between samples
    interval = 0
    while True:
        interval = input("Type the number of days interval between samples: ")
        try:
            number = float(interval)
            if number == 0:
                print("The number of days interval cannot be zero. Please try again.")
            else:
                break  # Break the loop if the input is a valid number and not zero
        except ValueError:
            # Catch the ValueError if conversion to float fails, which means the input was not a number
    ...
    return object_name, start_date, num_days, interval
#______________________________________________________________________________

def download_ephemeris(start_date, num_days, object_name):
    ...
    with open('downloaded_ephem_data.txt', 'w') as file:
...
def combine_data():
    obs_data = read_obs_data('downloaded_obs_data.txt')
    n_combinations = 0
    
    print('Combining data')
    with open('downloaded_ephem_data.txt', 'r') as eph_file, open('combined_data.txt', 'w') as out_file:
        for line in eph_file:
            parts = line.strip().split()
            date = ' '.join(parts[0:3])  # Combine year, month and day
            if date in obs_data:
                # If the date is in the observations file, we add the columns to the output file
                new_line = line.strip() + ' ' + ' '.join(obs_data[date]) + '\n'
                out_file.write(new_line)
                n_combinations = n_combinations + 1
        
        # Calculate number of successful combinations
        print(f"There have been {n_combinations} successful combinations.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        object_name, start_date, num_days, interval = user_input()
        
        #Downloadig ephemeris and observation data
        success_down_ephem = download_ephemeris(start_date, num_days, object_name)
