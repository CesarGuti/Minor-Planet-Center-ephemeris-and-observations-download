...
#
# The script combine ephemeris and observations data into a single file (stored
# in folder 'Total Observations'). This file contains the following columns: 
# year, month, day, observed magnitude, used filter, distance from Earth 
# (delta), distance from Sun (r) and orbit phase (alpha).
#
# Additionally, the data is reduced (stored in folder 'Reduced Observations'). 
# The dates are converted to days relative to perihelion, observations from 
# certain filters are discarded, the observed magnitudes are reduced by delta 
# and r, and a V band correction is also applied. This files contains the
# following columns: year, month, day, days relative to perihelion, delta, r,
# phase, observed magnitude and reduced magnitude.
# 
# From the execution, the graph of both the visual and reduced observations is 
# also obtained (stored in folder 'Plots'). This allows for quick and easy 
# previewing of data before more extensive analysis.
...
===============================================================================
"""
# This software is provided for scientific research purposes only. Any 
# commercial use or redistribution of this software without the express written
# consent of the author(s) is strictly prohibited. By using this software, you 
...
# are reduced. (v4)
#
# 2023-12-07 --> @Cesar G. de la Camara:The script is now capable of plotting 
# both visual and reduced observations. (v5)
...
def plot_reduced(name, start_date, end_date, perihelion_date, period):
    
    # Read the file
    data = pd.read_csv('reduced_data.txt', header=None, delimiter='\s+')

    # Selecting the required columns
    x = data.iloc[:, 3]  # Fourth column for x-axis (t-Tq)
    y = data.iloc[:, 8]  # Ninth column for y-axis (reduced obs)

    # Plotting
    plt.figure(figsize=(10, 6))
    plt.scatter(x, y, marker = 'o', s=3, color = 'red', alpha=0.7)
    plt.title(f"Reduced observations for {name}")
    plt.xlabel("t - Tq")
    plt.ylabel("m (1,1,alpha)")

    # Inverting the y-axis
    plt.gca().invert_yaxis()
    
    # Adding legend with the provided attributes
    perihelion_str = perihelion_date.strftime('%Y-%m-%d')
    period_str = str(round(period,2)) + " days"
    legend_text = f"Start Date: {start_date}\nEnd Date: {end_date}\nPerihelion: {perihelion_str}\nPeriod: {period_str}"
    plt.legend([legend_text], loc='upper left')
    
    # Save plot
    plot_target_folder = 'Plots'
    if not os.path.exists(plot_target_folder):
        os.makedirs(plot_target_folder)
        
    new_object_name = name.replace("\\", "_").replace("/", "_").replace(" ", "_")
    plot_name = f'{new_object_name}_reduced_observations_plot_from_{start_date}_to_{end_date}.png'
    plt.savefig(plot_name, bbox_inches='tight')
    
    # Mmove new plot
    os.rename(plot_name, os.path.join(plot_target_folder, plot_name))
#______________________________________________________________________________
def plot_total(name, start_date, end_date, perihelion_date, period):
    ...
    # Selecting the required columns
    x = data.iloc[:, 3]  # Fourth column for x-axis (t-Tq)
    y = data.iloc[:, 7]  # Eighth column for y-axis (total obs)

    # Plotting
    plt.figure(figsize=(10, 6))
    plt.scatter(x, y, marker = 'o', s=3, color='black', alpha=0.7)
    plt.title(f"Total observations for {name}")
    plt.xlabel("t - Tq")
    ...
    new_object_name = name.replace("\\", "_").replace("/", "_").replace(" ", "_")
    plot_name = f'{new_object_name}_total_observations_plot_from_{start_date}_to_{end_date}.png'
    plt.savefig(plot_name, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    print("This software is provided for scientific research purposes only. Any commercial use or redistribution of this software without the express written consent of the author(s) is strictly prohibited. By using this software, you agree to abide by these terms and conditions.")
    print("All right reserved to César Gutiérrez de la Cámara Jiménez and Ignacio Ramón Ferrín Vázquez.")
    ...
    while True:
        ...
        if success_down_obs == 0:
            ...
            if os.path.exists('downloaded_ephem_data.txt'):
                os.remove('downloaded_ephem_data.txt')
            break
        ...
        if n_obs != 0:
            ...
            # Plotting
            if n_obs_left != 0:    
                plot_reduced(object_name, start_date, end_date, perihelion_date, period)
            
            plot_total(object_name, start_date, end_date, perihelion_date, period)
            # Renaming files
        ...
        user_input = ask_y_or_n('Do you want to search another object?')
        if user_input == 'n':
            break
